\subsection{Categoria \textbf{Turing Machines}}
\label{subsec:turing}

\begin{definition}[Categoria Turing Machines]
Os \emph{objetos} são máquinas de Turing (determinísticas) finitas. Um morfismo $f: M_1 \to M_2$ é um par de mapas $(f_Q, f_\Gamma)$ --- sobre os conjuntos de estados e de símbolos de fita, respetivamente --- que preservam a função de transição, o estado inicial, os estados finais, e o símbolo em branco.
\end{definition}

\subsubsection{Representação dos Objetos}

Uma máquina de Turing $M = (Q, \Gamma, \delta, q_0, F, B)$ é codificada em MATLAB/Octave como uma \textit{struct}:

\begin{itemize}
    \item \texttt{Q} --- número de estados ($|Q|$), identificados por inteiros $1, \ldots, |Q|$;
    \item \texttt{Gamma} --- vetor de símbolos de fita (representados como inteiros; o símbolo em branco é o índice 1 por convenção);
    ...
    \item \texttt{blank} --- índice do símbolo em branco (por convenção, \texttt{blank = 1}).
\end{itemize}

\begin{lstlisting}[style=matlab, caption={Definição de uma máquina de Turing em MATLAB/Octave}]
...
\begin{definition}[Morfismo de Máquinas de Turing]
Um morfismo $f: M_1 \to M_2$ é um par $(f_Q, f_\Gamma)$ de mapas
\[
f_Q: Q_1 \to Q_2, \qquad f_\Gamma: \Gamma_1 \to \Gamma_2,
\]
tais que:
\begin{enumerate}
    ...
    \item Para cada $(q, a) \in Q_1 \times \Gamma_1$, se $\delta_1(q, a) = (q', a', d)$, então $\delta_2(f_Q(q), f_\Gamma(a)) = (f_Q(q'), f_\Gamma(a'), d)$ (preservação da função de transição).
\end{enumerate}
\end{definition}

\begin{lstlisting}[style=matlab, caption={Verificação de um morfismo de Máquinas de Turing}]
