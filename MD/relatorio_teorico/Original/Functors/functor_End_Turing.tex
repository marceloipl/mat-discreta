\subsection{Alínea 3.w) --- Functor $(\mathbf{End},\, \mathbf{Turing\ Machines})$}
\label{subsec:functor-end-turing}
\subsubsection{Functor Covariante $F: \mathbf{End} \to \mathbf{Turing\ Machines}$}

\begin{definition}[Functor covariante $F: \mathbf{End} \to \mathbf{Turing\ Machines}$]
...
\begin{proposition}
$F$ é covariante: $f \circ \varphi_1 = \varphi_2 \circ f$ implica $\delta_2(f_Q(q),f_\Gamma(a))=(f_Q(\varphi_1(q)),f_\Gamma(\varphi_1(a)),R)$, que é exatamente a condição de morfismo de \textbf{Turing\ Machines}.
\end{proposition}

\begin{lstlisting}[style=matlab, caption={Functor covariante $F: \mathbf{End} \to \mathbf{Turing\ Machines}$}]
...
\paragraph{Composto com $F_{\mathrm{TM}\to\mathrm{Struct}}$.}
...
\begin{lstlisting}[style=matlab, caption={Verificação da naturalidade $\eta^{(k_0)}: F \Rightarrow F^{(k_0)}$ em End $\to$ TM}]
...
\begin{lstlisting}[style=matlab, caption={Componente da trans. natural $\iota \Rightarrow F_{\mathrm{TM}\to\mathrm{Struct}} \circ F$ em $(X,\varphi)$}]
