% ============================================================
\section{Funtor End $\rightarrow$ Mon}
\label{sec:functor-end-mon}

\subsection{Fundamentação Teórica}

O funtor $F: \mathbf{End} \to \mathbf{Mon}$ associa a cada endomapa $\varphi$ o monoide das suas iterações distintas, capturando a estrutura algébrica do comportamento dinâmico de $\varphi$ a longo prazo. Como $X$ é finito, a sequência de iteradas estabiliza (torna-se eventualmente periódica), pelo que o monoide é sempre finito.

\subsection{Funtor Covariante End $\rightarrow$ Mon}

\begin{definition}[Funtor covariante $F: \mathbf{End} \to \mathbf{Mon}$]
...
\begin{lstlisting}[style=matlab, caption={Funtor covariante $F: \mathbf{End} \to \mathbf{Mon}$}]
...
\subsubsection{Adjunção com $U: \mathbf{Mon} \to \mathbf{End}$}
...
function eta_comp = apply_eta_Mon(Mon, k0)
    ...
    pk0 = Mon.identity;\input{Matemática Discreta/Report/Functors/21_End_Mon}
    for i = 1:k0
